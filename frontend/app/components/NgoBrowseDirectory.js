...
export default function NgoBrowseDirectory() {
  // Directory State
  ...
  const fetchNgos = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_URL}/api/forms/ngos`);
      if (res.data.success) {
  ...
  const handleCall = (ngo) => {
    const num = getCleanNumber(ngo.phone || ngo.whatsapp);
    if (!num) {
    ...
    Linking.openURL(`tel:${finalNum}`).catch(() => {
      setCallModal({ name: ngo.name, number: finalNum });
    });
  };
  ...
  const openMap = (ngoName, address, phone, formType) => {
    const refinedQuery = `${ngoName} (${formType || "NGO"}), ${address} ${
      phone ? `, ${phone}` : ""
    }, Pakistan`;
  ...
  const filteredNgos = ngos.filter((ngo) => {
    ...
    const matchesName = (ngo.name || ngo.ngoName || "")
      .toLowerCase()
      .includes(query);
    const matchesAddress = (ngo.address || "").toLowerCase().includes(query);
    const matchesServices = Array.isArray(ngo.category)
    ...
    <View style={styles.container}>
      ...
      <View style={styles.controlsSection}>
        ...
        <View style={styles.searchContainer}>
          ...
          <TextInput
            placeholder="Search by ngo name, city, or area..."
            ...
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptySubtitle}>
                {searchQuery !== ""
                  ...
                  : "No registered ngos are currently available in the directory."}
              </Text>
      ...
      {/* SERVICES LIST MODAL */}
      <Modal visible={!!selectedNgo} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeaderRow}>
              <View style={styles.modalHeaderTitleGroup}>
                <Text style={styles.modalTitle}>
                  {selectedNgo?.name || selectedNgo?.ngoName}
                </Text>
                <Text style={styles.modalSubTag}>Available Services</Text>
              </View>
              <TouchableOpacity
                style={styles.closeBtn}
                onPress={() => setSelectedNgo(null)}
              >
                <X size={18} color="#64748b" />
              </TouchableOpacity>
            </View>
            ...
            <ScrollView style={styles.servicesScroll}>
              {selectedNgo?.category && selectedNgo.category.length > 0 ? (
                selectedNgo.category.map((cat, idx) => (
