...
const ORG_TYPES = ["Registered NGO", "Trust", "Foundation", "Volunteer Group"];
const NGO_SERVICES = [
  "Ration Distribution",
  "Education Support",
  "Medical Camps",
  "Orphanage Support",
  "Clean Water (Wells/Filtration)",
  "Disaster Relief",
  "Women Empowerment",
  "Legal Aid",
  "Old Age Home",
  "Blood Donation Drive",
  "Dastarkhwan / Free Meals",
  "Skill Development",
];
...
export default function NgoRegistrationForm() {
  ...
  // --- Load Data into Form ---
  const loadNgoIntoForm = (item) => {
    if (!item) return;
    setSelectedNgoId(item._id);
    setRegisteredId(item._id);
    setIsLocked(true);

    reset({
      ngoName: item.name || item.ngoName || "",
  ...
  const fetchNgos = async () => {
    ...
    try {
      const res = await axios.get(`${apiUrl}/api/forms/get-ngo-data`, {
    ...
    try {
      if (registeredId) {
        ...
        const res = await axios.put(
          `${apiUrl}/api/forms/update-ngo/${registeredId}`,
          payload,
          { withCredentials: true },
        );
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await axios.delete(
        `${apiUrl}/api/forms/delete-ngo/${registeredId}`,
        { withCredentials: true },
      );

      showToast(res.data?.message || "NGO deleted", "success");
      handleAddNew();
      fetchNgos();
    } catch (err) {
      ...
      >
        <View style={styles.card}>
          ...
          <View style={styles.header}>
            <FontAwesome5 name="handshake" size={36} color="#FFFFFF" />
          ...
          {ngoList.length > 0 && (
            <View style={styles.registeredBar}>
              <Text style={styles.registeredBarTitle}>
                MY REGISTERED NGOS ({ngoList.length})
              </Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                <View style={{ flexDirection: "row", gap: 8 }}>
                  {ngoList.map((item, index) => {
                    const isSelected = item._id === selectedNgoId;
                    return (
                      <TouchableOpacity
                        key={item._id || index}
                        onPress={() => loadNgoIntoForm(item)}
                        style={[
                      ...
                      >
                        ...
                        >
                          {item.name || item.ngoName || `NGO #${index + 1}`}
                        </Text>
                      </TouchableOpacity>
                    );
                  })}
                  ...
                  <TouchableOpacity
                    ...
                    style={[
                      styles.registeredChip,
                      !selectedNgoId && styles.registeredChipNew,
                    ]}
                  >
                    <Feather
                      name="plus"
                      size={14}
                      color={!selectedNgoId ? "#FFF" : "#475569"}
                    />
                    <Text
                      style={[
                        styles.registeredChipText,
                        !selectedNgoId && styles.registeredChipTextSelected,
                      ]}
                    >
                      Add New
                    </Text>
          ...
          <View style={styles.formPadding}>
            ...
            <View style={styles.fieldContainer}>
              <Text style={styles.label}>
                <FontAwesome5 name="handshake" size={12} color="#DC2626" /> NGO
                NAME
              </Text>
              <Controller
                control={control}
                ...
                render={({ field: { onChange, value } }) => (
                  <TextInput
                    style={[
                      ...
                      errors.ngoName && styles.inputError,
                    ]}
                    placeholder="e.g. Shoukat Khanum NGO"
              ...
              />
              {errors.ngoName && (
                <Text style={styles.errorText}>{errors.ngoName.message}</Text>
              )}
            </View>
            ...
            <View style={styles.actionsContainer}>
              {!registeredId ? (
                ...
                >
                  <Text style={styles.primaryButtonText}>Register NGO</Text>
                </TouchableOpacity>
              ) : (
                <View style={{ gap: 10 }}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.deleteModalContent}>
            ...
            <Text style={styles.deleteSubtitle}>
              Do you really want to delete this ngo record? This action cannot
              be undone.
            </Text>
