...
//   StyleSheet,
// } from "react-native";
// import { Search, Building2, RefreshCw, XCircle } from "lucide-react-native";
// import HospitalCard from "./HospitalCard";
// import SkeletonCard from "./ui/SkeletonCard";

// export default function HospitalBrowseDirectory({
//   hospitals,
//   searchQuery,
//   setSearchQuery,
//   loading,
...
//     const matchesName = (hosp.name || hosp.hospitalName || "")
//       .toLowerCase()
//       .includes(query);
//     const matchesAddress = (hosp.address || "").toLowerCase().includes(query);
//     const matchesServices = Array.isArray(hosp.category)
//       ? hosp.category.some((cat) => cat.toLowerCase().includes(query))
//       : false;
...
//                   : "No registered hospitals are currently available in the directory."}
//               </Text>
...
export default function HospitalBrowseDirectory() {
  ...
  const handleCall = (hosp) => {
    const num = getCleanNumber(hosp.phone || hosp.whatsapp);
    if (!num) {
    ...
    Linking.openURL(`tel:${finalNum}`).catch(() => {
      setCallModal({ name: hosp.name, number: finalNum });
    });
  };
