...
export default function AmbulanceBrowseDirectory() {
  // Directory State
  ...
  const fetchAmbulances = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_URL}/api/forms/ambulances`);
      if (res.data.success) {
  ...
  const handleCall = (ambulance) => {
    const num = getCleanNumber(ambulance.phone || ambulance.whatsapp);
    if (!num) {
    ...
    Linking.openURL(`tel:${finalNum}`).catch(() => {
      setCallModal({ name: ambulance.name, number: finalNum });
    });
  };
  ...
  const openMap = (ambulance, address, phone, formType) => {
    const refinedQuery = `${ambulance.name} (${formType || "Ambulance"}), ${address} ${
      phone ? `, ${phone}` : ""
    }, Pakistan`;
  ...
  const filteredAmbulances = ambulances.filter((ambulance) => {
    ...
    const matchesName = (ambulance.name || "").toLowerCase().includes(query);
    const matchesAddress = (ambulance.address || "")
      .toLowerCase()
      .includes(query);
    const matchesServices = Array.isArray(ambulance.category)
      ? ambulance.category.some((cat) => cat.toLowerCase().includes(query))
      : false;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.controlsSection}>
        ...
        <View style={styles.searchContainer}>
          ...
          <TextInput
            placeholder="Search by ambulance name, city, or area..."
            ...
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptySubtitle}>
                {searchQuery !== ""
                  ...
                  : "No registered ambulances are currently available in the directory."}
              </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeaderRow}>
              <View style={styles.modalHeaderTitleGroup}>
                <Text style={styles.modalTitle}>
                  {selectedAmbulance?.ambName || selectedAmbulance?.name}
                </Text>
            ...
            <ScrollView style={styles.servicesScroll}>
              {selectedAmbulance?.category &&
              selectedAmbulance.category.length > 0 ? (
                selectedAmbulance.category.map((cat, idx) => (
