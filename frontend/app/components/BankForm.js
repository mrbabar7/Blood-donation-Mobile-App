...
const schema = z.object({
  bankName: z.string().min(3, "Blood Bank Name is required"),
...
export default function BankRegistrationForm() {
  ...
  // --- Load Data into Form ---
  const loadBankIntoForm = (item) => {
    if (!item) return;
    setSelectedBankId(item._id);
  ...
  const fetchBanks = async () => {
    ...
    try {
      const res = await axios.get(`${apiUrl}/api/forms/get-bloodbank-data`, {
    ...
    try {
      if (registeredId) {
        ...
        const res = await axios.put(
          `${apiUrl}/api/forms/update-bloodbank/${registeredId}`,
          payload,
          { withCredentials: true },
        );
        ...
        const res = await axios.post(
          `${apiUrl}/api/forms/bloodbank-register`,
          payload,
          { withCredentials: true },
        );
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await axios.delete(
        `${apiUrl}/api/forms/delete-bloodbank/${registeredId}`,
        { withCredentials: true },
      );

      showToast(res.data?.message || "Blood Bank deleted", "success");
      handleAddNew();
      fetchBanks();
    } catch (err) {
      ...
      >
        <View style={styles.card}>
          ...
          <View style={styles.header}>
            <FontAwesome5 name="hand-holding-heart" size={36} color="#FFFFFF" />
          ...
          {bankList.length > 0 && (
            <View style={styles.registeredBar}>
              <Text style={styles.registeredBarTitle}>
                MY REGISTERED BLOOD BANKS ({bankList.length})
              </Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                <View style={{ flexDirection: "row", gap: 8 }}>
                  {bankList.map((item, index) => {
                    const isSelected = item._id === selectedBankId;
                    return (
                      <TouchableOpacity
                        key={item._id || index}
                        onPress={() => loadBankIntoForm(item)}
                        style={[
                  ...
                  <TouchableOpacity
                    ...
                    style={[
                      styles.registeredChip,
                      !selectedBankId && styles.registeredChipNew,
                    ]}
                  >
                    <Feather
                      name="plus"
                      size={14}
                      color={!selectedBankId ? "#FFF" : "#475569"}
                    />
                    <Text
                      style={[
                        styles.registeredChipText,
                        !selectedBankId && styles.registeredChipTextSelected,
                      ]}
                    >
                      Add New
                    </Text>
          ...
          <View style={styles.formPadding}>
            ...
            <View style={styles.fieldContainer}>
              <Text style={styles.label}>
                <FontAwesome5 name="suitcase" size={12} color="#DC2626" /> BLOOD
                BANK NAME
              </Text>
              <Controller
                control={control}
                name="bankName"
                // rules={{ required: "Blood bank name is required" }}
                render={({ field: { onChange, value } }) => (
                  <TextInput
                    ...
                    placeholder="e.g. Shoukat Khanum Blood Bank"
            ...
            <View style={styles.fieldContainer}>
              <Text style={styles.label}>
                ...
                BLOOD BANK SERVICES & FACILITIES
              </Text>
              <TouchableOpacity
                disabled={isLocked}
                style={[
              ...
              <Controller
                ...
                render={({ field: { onChange, value } }) => (
                  <TextInput
                    ...
                    placeholder="https://www.bloodbank.com"
                    placeholderTextColor="#A0AEC0"
                    autoCapitalize="none"
                    keyboardType="url"
                    value={value}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.deleteModalContent}>
            ...
            <Text style={styles.deleteSubtitle}>
              Do you really want to delete this blood bank record? This action
              cannot be undone.
            </Text>
